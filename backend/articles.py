...
def board():
    posts=board_post.query.limit(4).all()
...
def select(num):
    selected_post=board_post.query.filter_by(seq=num).all()
    if selected_post:
        return render_template('articles.html', posts=selected_post)
    else:
        return "Post not found"

#POST 게시물 DB(post)에 저장
...
def post():
    ...
    new_post=board_post(title=title,content=content, writer_id=writer_id,create_date=create_date)
